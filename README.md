...
- **服务端权威状态**:✅ 真实部署/还原 release 由 Console 服务端 `appendRelease` 权威落库(前端不伪造);启停走真机 `POST …/lifecycle`(systemd/pm2),用 Agent 返回的真实 active/pid 刷新,不再前端随机 pid/cpu/mem;运行态由应用详情轮询 Agent `…/status`。残留:app 实体的 status/version 仍由前端在操作成功后 patch 落库(基于 Agent 权威结果),尚未拆为独立的服务端落库 API——属展示层收口,非数据正确性问题。
- **幂等隔离**:✅ Agent 本地按 `op/appId/releaseId` 命名空间去重 + 同应用串行锁;Console `deploys` 表主键亦为 `(op, app_id, release_id)` 复合键(旧单列库启动自动迁移),部署/还原、不同应用复用同一 releaseId 互不误命中。
...
- **Deployer 完整度**:✅ static-nginx 还原(软链切回历史 release)、tomcat/static UI 真机部署接入(isRealType 全开)、python/node 多文件包、reload 钩子(static→nginx-reload / tomcat→tomcat-restart,服务端按类型映射白名单动作、前端只表意)均已完成。剩 requirements 自动安装、tomcat/static 运行时日志的前端路径选择器待补。
...
- **运行态查询**:✅ 应用详情对真实进程类应用 10s 轮询 Agent `…/status`,展示真实 active/pid;资源指标(CPU/内存)Agent 暂不采集,明确标注未采集而非伪造。
