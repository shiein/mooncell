...
| Deployer:python(单文件入口 / 多文件包 · python3 / venv + systemd / pm2)| ✅ 完成(真机验证:部署→还原→日志→回滚闭环;多文件 tar.gz 包 + requirements 自动安装均已支持)|
...
| 应用运行时日志实时流(Agent 跟随 systemd journal → SSE → Console 代理 → 前端 tail+跟随)| ✅ 完成(go-binary 真机验证:直连/经代理双路、断开级联取消;真实应用流失败显示错误态+重试,不伪造模拟;仅演示类型用模拟)|
...
- **Deployer 完整度**:✅ static-nginx 还原(软链切回历史 release)、tomcat/static UI 真机部署接入(isRealType 全开)、python/node 多文件包、reload 钩子(static→nginx-reload / tomcat→tomcat-restart,服务端按类型映射白名单动作、前端只表意)、声明日志文件的前端 tail 选择(Console 按 logPaths 授权)、python 多文件包 requirements 自动安装均已完成。
- **文件柜**:✅ 二进制落盘 + 提取码 + 公开直链 + 凭码下载 + 匿名上传(`cabinet.anon_upload` 开关,免登录 POST /api/pub/cabinet)+ 过期自动清理(后台每小时删元数据 + 字节)。
- **真实运行时验收**:✅ pm2 与 Tomcat 均已在真机(便携 node+pm2 / 便携 JRE+Tomcat,隔离 `/opt`、测试后零残留)完成**真实运行时**验收——
  - pm2:node 应用经 pm2 runner 部署 → `pm2 list` 显示 online、真实 pid/cpu/mem,健康探活 200;
  - tomcat-war:WAR 经 tomcat runner 部署(原子换 WAR + 清展开目录)→ 真实 Tomcat 自动展开并 serve(`/app/` → 200)。
  另有假 pm2 集成测试 `TestRunDeployPm2Orchestration` 兜底编排逻辑。
- **大文件 / 不稳网络验收**:✅ 真机 150MB 制品:分块上传中途「中断」后凭 `nextIndex` 续传至完整、Console 组装后单次 multipart 发 Agent、**落盘 sha 字节级一致**、上传临时零残留。
