    ...
    <div>
      ...
      {!noRealSource ? (
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 11.5, color: "var(--muted-fg)" }}>
          <span className="mono">{useReal ? (followingFile ? `tail -F ${logSrc}` : (app.runner === "pm2" ? `pm2 logs ${(app.pm2Name || "").trim() || "deploy-" + app.id}` : app.runner === "nohup" ? `tail -F ${(app.logPaths && app.logPaths[0]) || (app.path + ".nohup.log")}` : `journalctl -u deploy-${app.id}`)) : (app.logPaths && app.logPaths[0]) || "—"}</span>
          <span>缓冲 {lines.length} / 400 行 · {useReal ? ((followingFile || app.runner === "nohup") ? "tail -F 跟随(轮转安全)" : "journald 跟随(轮转安全)") : "轮转安全(fsnotify 重开文件)"}</span>
        </div>
      ) : null}
    </div>
  );
  ...
  return (
    <div className="card card-pad" style={{ maxWidth: 760 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
        ...
        {draft.runner === "pm2" ? (
          <Field label="pm2 接管进程名(可选)" hint="填已有 pm2 进程名/ID → 部署自动定位该进程实际运行的制品并替换 + pm2 restart(Java 取启动参数里 -jar 的 jar,node/python/原生取 pm_exec_path),无需对齐路径;留空 = Mooncell 托管(deploy-<id>)">
