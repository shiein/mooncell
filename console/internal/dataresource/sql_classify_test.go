...
func TestClassifySQL(t *testing.T) {
	cases := []struct {
		...
		{"WITH d AS (DELETE FROM t RETURNING id) SELECT * FROM d", StmtDelete},
		{"WITH i AS (INSERT INTO t VALUES (1) RETURNING id) SELECT * FROM i", StmtInsert},
		{"WITH u AS (UPDATE t SET a=1 RETURNING id) SELECT * FROM u", StmtUpdate},
		{"WITH cte AS (SELECT 1) INSERT INTO t SELECT * FROM cte", StmtInsert},
		{"  -- comment\nSELECT 1", StmtSelect},
