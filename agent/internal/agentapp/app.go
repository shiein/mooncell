...
// agentVersion 由根 package main 的构建版本传入，供版本接口和自更新审计使用。
...
type agent struct {
	...
	opsInFlight  int          // 在飞状态变更操作数;自更新前置 draining 并等其清零,避免 self-exec 打断在飞部署
	draining     bool         // 自更新 draining:置位后 beginOp 拒绝新操作
}
...
// Run 启动 Agent 应用。根 package main 只保留稳定的可执行入口和构建版本注入点。
