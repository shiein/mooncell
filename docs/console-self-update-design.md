...
- `--selftest`:**只** `loadConfig("config.toml")`(校验配置可解析 + 通过 `unsafeConsoleConfigReason`
  ...
  - **不校验内嵌 dist 完整性**(决策):`go:embed` 是编译期保证,运行期再校验无收益,保持最简。
...
**新开一个 admin-only「系统」页**(决策),承载 Console 自更新,并预留未来系统级操作(版本信息、
DB 备份/下载、运行参数等)。导航入口加到 `console/src/components/Shell.jsx`,仅 admin 可见
(与「用户管理」「Agent 管理」同级,`store.can("admin")` 门控)。

页内一张「Console 升级包」卡片,仿现有「Agent 升级包」卡(`console/src/pages/Agents.jsx` 的
`AgentBinariesCard`):

- 顶部显示当前 Console 版本 + os/arch(读 `GET /api/console/info`)。
- 文件选择 + 「上传并升级」按钮 → `confirmDialog` 二次确认。确认弹窗须**醒目提示先备份
  `mooncell.db`**(决策:跨版本迁移风险),并明确告知**将就地重启、约数秒断连、在飞操作(部署/上传/
  巡检)会中断**。
...
> 「系统」页后续可扩展:一键下载 `mooncell.db` 备份(与自更新前备份提示形成闭环)。
...
3. **DB 不在自更新范围**:只换二进制。新版本若改表结构,靠重启后 `openDB` 的
   `CREATE TABLE IF NOT EXISTS` / 迁移处理(与现有升级一致)。**自更新前的确认弹窗会强制提示先备份
   `mooncell.db`**(决策,见 §4),尤其跨大版本;「系统」页后续提供一键备份下载。
...
5. 前端:`api.js` 加 `consoleSelfUpdate(file, version)` / `getConsoleInfo()`;**新开「系统」页**
   (`console/src/pages/System.jsx`)放「Console 升级包」卡;`Shell.jsx` 加 admin-only 导航入口 +
   `App.jsx` 路由。确认弹窗含**备份 `mooncell.db` 提示**。
...
1. **前端入口**:新开 admin-only「系统」页承载自更新(预留版本/DB 备份等系统级操作),不挂在 Agent 页。
2. **备份 DB**:自更新确认弹窗强制提示先备份 `mooncell.db`(跨版本迁移风险);「系统」页后续提供一键备份下载。
3. **`--selftest` 不校验内嵌 dist**:go:embed 编译期保证,运行期再校验无收益,保持最简(只 `loadConfig`)。
